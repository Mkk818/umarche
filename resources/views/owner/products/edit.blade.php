    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
          <x-auth-validation-errors class="mb-4  flex flex-col items-center" :errors="$errors" />
          <form method="POST" action="{{ route('owner.products.update', ['product' => $product->id ])}}">
            ...
            <div class="-m-2">
              ...
              <div class="relative flex justify-evenly">
                <div>
                  <input type="radio" name="type" value="1" class="mr-2" @if($product->is_selling === 1){checked } @endif>追加
                </div>
                <div>
                  <input type="radio" name="type" value="2" class="mr-2" @if($product->is_selling === 0){checked } @endif>削減
                </div>
              </div>
              <div class="p-2 w-1/2 mx-auto">
                <div class="relative">
                  <label for="quantity" class="leading-7 text-sm text-gray-600">数量※必須</label>
                  <input type="number" id="quantity" name="quantity" value="０" required
                    class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
              ...
              <div class="p-2 w-1/2 mx-auto">
                <div class="relative">
                  <label for="shop_id" id="shop_id" class="leading-7 text-sm text-gray-600">販売店舗</label>
              ...
              <x-select-image :images="$images" currentId="{{ $product->image5 }}" currentImage="{{ $product->imageFirst->filename ?? ''}}" name="image5" />
              <div class="p-2 w-1/2 mx-auto">
